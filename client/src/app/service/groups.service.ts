...
export class GroupsService {
  ...
  getAllAnimalTypes(): Observable<Animaltype[]> {
    const results: Observable<Animaltype[]> = this.http.get<Animaltype[]>(
  ...
  getAllAnimalTypesByShelter(shelterId: number): Observable<Animaltype[]> {
    const results: Observable<Animaltype[]> = this.http.get<Animaltype[]>(
      this.dataUrl + `/animaltypes/byshelter/${shelterId}`
    );
    return results;
  }
  ...
  deleteAnimalTypeById(animalTypeId: number): Observable<Animaltype> {
    const results: Observable<Animaltype> = this.http.delete<Animaltype>(
      this.dataUrl + `/animaltypes/${animalTypeId}`
    );
    return results;
  }
