...
export class MembersService {
  ...
  getAllAnimals(): Observable<Animal[]> {
    const results: Observable<Animal[]> = this.http.get<Animal[]>(
  ...
  getAllAnimalsByAnimalType(animalTypeId: number): Observable<Animal[]> {
    const results: Observable<Animal[]> = this.http.get<Animal[]>(
      this.dataUrl + `/animals/byanimaltype/${animalTypeId}`
    );
    return results;
  }
  ...
  deleteAnimalById(animalId: number, animalTypeId: number): Observable<Animal> {
    const results: Observable<Animal> = this.http.delete<Animal>(
      this.dataUrl + `/animaltypes/${animalTypeId}/animals/${animalId}`
    );
    return results;
  }
